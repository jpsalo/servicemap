...
define(function() {
    var DataVisualization = (function() {
        let HEATMAP_DATASETS = undefined;
        let STATISTICS_DATASETS = undefined;
        let FORECAST_DATASETS = undefined;
        let STATISTICS_TYPES = undefined;
        let DATA_LAYERS = undefined;
        DataVisualization = class DataVisualization {
            ...
            heatmapLayerPath(id) {
                ...
                if (!HEATMAP_DATASETS[id]) {
                    return '';
                }
                return `https://geoserver.hel.fi/geoserver/gwc/service/wmts/?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=popdensity:alltest&style=popdensity:all_density_${HEATMAP_DATASETS[id]}&FORMAT=image%2F${layerFmt}&TILEMATRIXSET=ETRS-TM35FIN&TILEMATRIX=ETRS-TM35FIN:{z}&TILEROW={y}&TILECOL={x}`;
            }

            getStatisticsLayer(name) {
                return STATISTICS_DATASETS[name];
            }

            getStatisticsType(name) {
                return STATISTICS_TYPES[name];
            }

            getHeatmapLayers() {
                return Object.keys(HEATMAP_DATASETS);
            }

            getStatisticsLayers() {
                return Object.keys(STATISTICS_DATASETS);
            }

            getForecastsLayers() {
                return Object.keys(FORECAST_DATASETS);
            }
        };
        DataVisualization.initClass();
